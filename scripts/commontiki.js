...
function setEvents() {
		MapFrame.document.onmousemove = trackM;
		MapFrame.document.onmousedown = whichTool;
		MapFrame.document.onmouseup = stop;
		MapFrame.document.onmouseout = clear;
}

//clear crosshair
function clear(){
	MapFrame.crossH.clear();
	MapFrame.crossV.clear();
	return false;
}
...
function trackM(e) {
	getIxIy(e);
	//coordinate display
//	getXYasMapCoordinates(xPosition,yPosition);
//	document.coordinateDisplay.x.value = mapX;	
//	document.coordinateDisplay.y.value = mapY;	
	//draw crosshair
	MapFrame.crossH.clear();
	MapFrame.crossV.clear();
	if (wX > posleft && wY > postop && wX < posleft+layerwidth && wY < postop+layerheight){
		MapFrame.crossH.drawLine(0,yPosition,MapWidth,yPosition);
		MapFrame.crossV.drawLine(xPosition,0,xPosition,MapHeight);
		MapFrame.crossH.paint();
		MapFrame.crossV.paint();
	}
	//
	if (zooming) {
		MapFrame.jg.clear(); 
		...
		pointsY[3]=currentY;
		pointsY[4]=beginY;
		MapFrame.jg.drawPolyline(pointsX, pointsY);
		MapFrame.jg.paint();
		return false;
	}else if (panning){
		if (isNav6) {
...
function getIxIy(e) {
	if (isNav6) {
		...
		wX=MapFrame.event.clientX  + MapFrame.document.body.scrollLeft;
		wY=MapFrame.event.clientY  + MapFrame.document.body.scrollTop;
	}
...
function getXYasMapCoordinates(x,y){
	currentExtent = new Array();
	currentExtent = parseImgExt(imgext);
	var deltaX = (currentExtent[2]-currentExtent[0]);
	var deltaY = (currentExtent[3]-currentExtent[1]);
	pixelSizeX = deltaX/layerwidth;
